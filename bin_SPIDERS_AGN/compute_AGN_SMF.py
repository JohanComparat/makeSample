...
import XrayAgnBo16Model
xr = XrayAgnBo16Model.XrayAgnBo16Model()
...
z_max = 0.35 #float(sys.argv[2]) #1.5
percentage_allowed = 50. #float(sys.argv[3]) # 20.
...
f = h5py.File('/data17s/darksim/MD/MD_1.0Gpc/h5_lc/lc_C1.hdf5', 'r')
...
sel = (is_agn)&(f['/sky_position/redshift_S'].value>z_min)&(f['/sky_position/redshift_S'].value<z_max)&(n.log10(f['/stellar_mass_data/stellar_mass'].value)+f['/agn_properties/log_lambda_sar'].value>n.log10(LX_min))

area = 3044.0
volume_sim = volume * area *n.pi / 129600. 

logM = n.log10(f['/stellar_mass_data/stellar_mass'].value[sel])
