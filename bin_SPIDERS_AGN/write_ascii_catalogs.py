...
LX_limit = dl_lim.value**2.* 10**(-12.7)* 4 * np.pi
lx_2_z_max = interp1d(LX_limit, z_s)

def write_ascii_clusterings_catalog(out_file, ratelim_min = 0.01, hemisphere='N', lx_min=1e44):
  ...
  np.savetxt(out_file+str(ratelim_min)+"_LX_"+str(int(np.log10(lx_min)*10))+"_"+hemisphere+".data", np.transpose([ ra_data[sel_data], dec_data[sel_data], z_data[sel_data], np.ones_like(z_data[sel_data]) ]))
  np.savetxt(out_file+str(ratelim_min)+"_LX_"+str(int(np.log10(lx_min)*10))+"_"+hemisphere+".random", np.transpose([ ra_rds[sel_rds], dec_rds[sel_rds], RR, np.ones_like(RR) ]))
