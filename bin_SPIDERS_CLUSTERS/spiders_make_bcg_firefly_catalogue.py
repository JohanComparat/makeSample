...
cat = fits.open(os.path.join(os.environ['DATA_DIR'], 'spiders', 'cluster', 'mastercatalogue_FINAL_CODEXID.fits'))[1].data
dir2 = os.path.join(os.environ['HOME'], 'data', 'spiders', 'cluster')
in_cat = os.path.join(dir2, 'mastercatalogue_FINAL_CODEXID-flat_DR16_firefly.fits')
out_cat = os.path.join(dir2, 'mastercatalogue_FINAL_CODEXID-flat_DR16_firefly-massBCG.fits')

all_clus_id = n.array(list(set(cat['CLUS_ID'])))
all_clus_id.sort()

hd = fits.open(in_cat)[1].data

uniq_clus_id = n.unique(hd['# CLUS_ID'])
mass_bcg = n.zeros_like(hd['Chabrier_ELODIE_total_mass'])
mass_bcg2 = n.zeros_like(hd['Chabrier_ELODIE_total_mass'])
for el in uniq_clus_id:
	#print('====================================')
	#print(el)
	mass = n.zeros_like(hd['Chabrier_ELODIE_total_mass'])
	mass2 = n.zeros_like(hd['Chabrier_ELODIE_total_mass'])
	sel = (hd['# CLUS_ID']==el)&(hd['SCREEN_ISMEMBER_W']>0)
	mass[sel] = hd['Chabrier_ELODIE_total_mass'][sel]
	mass2[sel] = hd['Chabrier_MILES_total_mass'][sel]
	id_max = n.argmax(mass)
	id_max2 = n.argmax(mass2)
	mass_bcg[id_max] = n.max(mass)
	mass_bcg2[id_max2] = n.max(mass2)
	#print(el, hd['Chabrier_ELODIE_total_mass'][sel], mass_bcg[sel])
	#for jj,kk in zip(hd['Chabrier_ELODIE_total_mass'][sel], mass_bcg[sel]):
		#print(jj,kk)

t = Table()
t.add_column(Column(name='CLUS_ID', data=hd['# CLUS_ID']))
for name in hd.columns.names[1:]:
	t.add_column(Column(name=name, data=hd[name]))

t.add_column(Column(name='Chabrier_ELODIE_total_mass_BCG', data=mass_bcg, unit='Msun'))
t.add_column(Column(name='Chabrier_MILES_total_mass_BCG', data=mass_bcg2, unit='Msun'))

t.write(out_cat, overwrite=True)
...
DATA = [] 
for cc in cat:
	delta_z, sep_r200c, stellar_mass , is_mem, ages = get_props(cc, spm26, 'Z')
	mem = (is_mem==1)
...
n.savetxt(os.path.join(os.environ['DATA_DIR'], 'spiders', 'cluster', 'phase-space_26.dat'), ttt)
